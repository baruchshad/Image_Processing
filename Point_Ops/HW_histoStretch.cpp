...
HW_histoStretch(ImagePtr I1, int t1, int t2, ImagePtr I2)
{
	...
	int i, lut[MXGRAY];
	int range = t2 - t1;				
	if (range == 0) std::cout << "None valid input for t1 and t2 values" << endl;									// enforce t2 - t1 be greater then zero
	for (i = 0; i <= t1; ++i) lut[i] = 0;			// sets all the bits under t1 to be 0
	for (; i < t2; ++i) {
		lut[i] = (MXGRAY - 1) * (i - t1) / range;	// Computes the equation from the book [g = 255(f-min)/(max-min)]
	}
	for (; i < MXGRAY; ++i) lut[i] = MXGRAY - 1;	// sets any value greater than or equal to the maximum gray.
